class GithubService
  ...
  def initialize(access_hash = nil)
    @access_token = access_hash['access_token'] || access_hash[:access_token] unless access_hash.nil?
  end

  def authenticate!(client_id, client_secret, code)
    response = Faraday.get "https://github.com/login/oauth/access_token",
  ...
  def get_username
    ...
    user_json = JSON.parse(user_response.body)['login']
